ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :message_services, :collection =>{:render_message_template => :get,:student_groups => :get,:mobile_number => :any},:member => {:status_update => :any}
  map.resources :students
  map.login "login", :controller => "user_sessions", :action => "new"
  map.logout "logout", :controller => "user_sessions", :action => "destroy"
  map.resources :user_sessions
  map.resources :users,:collection =>{:forgot_password =>:any}
  map.root :controller => 'message_services', :action => 'new'
end
